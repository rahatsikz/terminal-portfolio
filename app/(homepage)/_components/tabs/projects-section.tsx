...
export default function ProjectsSection() {
  return (
    <motion.div
      ...
      className='space-y-6'
    >
      <div className='text-green-400 text-xl font-bold mb-5 mt-3.5 pl-1.5'>
        Projects
      </div>
        ...
        {projectsArr.map((project, index) => (
          ...
          >
            <Card className='bg-gray-800 border-gray-700 overflow-hidden'>
              <div className='border-y h-[270px] border-input/5 flex justify-center items-center bg-gray-700 overflow-hidden'>
                <Image
                  src={project.image || "/placeholder.svg"}
                  alt={project.title}
                  sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw'
                  className='w-[calc(100%-1.5rem)] m-auto h-60  object-cover rounded-md'
                  width={300}
                  height={200}
                />
              </div>
              ...
              <CardFooter className='flex gap-3'>
                <Link
                  target={project.github.length ? "_blank" : "_self"}
                  href={project.github}
                >
      ...
      <div className='bg-gray-800 p-4 rounded border border-gray-700'>
        <pre className='text-gray-300 whitespace-pre-wrap leading-8'>
          {`$ git log --oneline -n 3
a1b2c3d 🚀 Launched latest project: Avena
